...
export class TestRailReportsAPI extends TestRailBase {
	...
	async getReports(projectId: number): Promise<Report[]> {
		return this.makeRequest<Report[]>(`get_reports/${projectId}`);
	}
	 ...
	 */
	async runReport(reportTemplateId: number): Promise<RunReportResponse> {
		return this.makeRequest<RunReportResponse>(`run_report/${reportTemplateId}`);
	}
}
