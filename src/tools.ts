...
export function registerTools(server: McpServer, api: TestRailAPI) {
				...
				if (projects.length === 0) {
					return {
						content: [
							{
								type: "text",
								text: "No TestRail projects found or no access to projects.",
							},
						],
					};
				}
			...
			try {
				...
				return {
					content: [
						{
							type: "text",
							text: `## 🏗️ Project: ${project.name}\n\n` + `**ID:** ${project.id} | **Status:** ${project.is_completed ? "✅ Completed" : "🟢 Active"}\n\n` + `${project.announcement ? `📢 **Description:** ${project.announcement}\n\n` : ""}` + `### 🗂️ Test Suites (${suitesArray.length} total):\n\n` + (suitesArray.length > 0 ? suitesArray.map((s) => `• **${s.name}** (ID: ${s.id})\n  ${s.description || "_No description provided_"}`).join("\n\n") + "\n\n" : "_No test suites found in this project._\n\n") + `💡 **Next steps:**\n` + `• Use \`#commands list_sections project_id: ${project.id} suite_id: [SUITE_ID]\` to explore sections\n` + `• Use \`#commands create_section suite_id: [SUITE_ID] name: "Section Name"\` to add sections\n` + `• Use \`#commands list_test_cases project_id: ${project.id} suite_id: [SUITE_ID] section_id: [SECTION_ID]\` to see test cases`,
						},
					],
				};
			} catch (error) {
				return {
					content: [
						{
							type: "text",
							text: `❌ **Error:** ${error}\n\n` + `Failed to get project details for project ID ${project_id}. Please check that the project ID is correct and you have access to it.`,
						},
					],
				};
			}
	...
	server.registerTool(
		...
		async ({ project_id, suite_id, section_id }) => {
			try {
				const cases = await api.getCases(project_id, suite_id, section_id);
				...
				if (cases.length === 0) {
					return {
						content: [
							{
								type: "text",
								text: `## 📋 No Test Cases Found\n\n` + `No test cases found in section ${section_id}.\n\n` + `💡 **Next steps:**\n` + `• Use \`#create_text_test_case section_id: ${section_id}\` to create a simple test case\n` + `• Use \`#create_steps_test_case section_id: ${section_id}\` to create a detailed test case\n` + `• Use \`#create_exploratory_test_case section_id: ${section_id}\` to create an exploratory test case`,
							},
						],
					};
				}

				const caseTypeMap = { 1: "Text", 2: "Steps", 3: "Exploratory" };
				...
				return {
					content: [
						{
							type: "text",
							text: `## 🧪 Test Cases in Section ${section_id}\n\n` + `Found **${cases.length} test cases**:\n\n` + cases.map((c: any) => `### ${c.title} (ID: ${c.id})\n` + `• **Type:** ${caseTypeMap[c.type_id as keyof typeof caseTypeMap] || "Unknown"}\n` + `• **Priority:** ${priorityMap[c.priority_id as keyof typeof priorityMap] || "Unknown"}\n` + `${c.estimate ? `• **Estimate:** ${c.estimate}\n` : ""}` + `• **Created:** ${c.created_on ? new Date(c.created_on * 1000).toLocaleDateString() : "Unknown"}\n` + `• **Status:** ${c.is_deleted ? "❌ Deleted" : "✅ Active"}`).join("\n\n") + "\n\n" + `💡 **Next steps:**\n` + `• Use \`#get_test_case case_id: [ID]\` to see full details of a test case\n` + `• Use \`#create_test_run project_id: ${project_id}\` to create a test run\n` + `• Use \`#copy_test_cases case_ids: [IDs] target_section_id: [ID]\` to copy test cases`,
						},
					],
				};
			} catch (error) {
				return {
					content: [
						{
							type: "text",
							text: `❌ **Error:** ${error}\n\n` + `I need the following parameters to list test cases:\n` + `• **project_id** - Use \`#list_projects\` to see available projects\n` + `• **suite_id** - Use \`#get_project project_id: [ID]\` to see suites\n` + `• **section_id** - Use \`#list_sections project_id: [ID] suite_id: [ID]\` to see sections\n\n` + `**Once you have all IDs, use:**\n` + `\`#list_test_cases project_id: [PROJECT_ID] suite_id: [SUITE_ID] section_id: [SECTION_ID]\``,
						},
					],
				};
			}
				...
				content: [
					{
						...
						text:
							...
							`${testCase.custom_steps_separated ? `**Test Steps:**\n` + testCase.custom_steps_separated.map((step: any, i: number) => `${i + 1}. **Action:** ${step.content}\n   **Expected:** ${step.expected}`).join("\n") + "\n\n" : ""}` +
					...
					.array(
						z.object({
							...
							steps_separated: z
								...
								.optional()
								.describe("Separated steps (for steps type)"),
					...
					content: [
						{
							type: "text",
							text:
								`# 🚨 DELETE PREVIEW - REVIEW CAREFULLY\n\n` +
					...
					content: [
						{
							type: "text",
							text:
								`# 🎯 Milestones for Project ${project_id}\n\n` +
					...
					content: [
						{
							type: "text",
							text:
								`# 🏃 Test Runs for Project ${project_id}\n\n` +
								`**Found ${runs.runs?.length || 0} test runs:**\n\n` +
								(runs.runs?.length > 0
									? runs.runs
											.map((run: any) => {
												const status = run.is_completed ? "✅ Completed" : "🟡 Active";
												const passRate = run.passed_count && run.passed_count + run.failed_count > 0 ? Math.round((run.passed_count / (run.passed_count + run.failed_count)) * 100) : 0;
