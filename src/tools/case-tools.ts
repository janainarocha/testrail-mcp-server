...
export function registerCaseTools(server: McpServer, clients: { cases: TestRailCasesAPI; caseFields: TestRailCaseFieldsAPI; priorities?: TestRailPrioritiesAPI }) {
	/**
				...
				const options = Object.entries(params)
					...
					.reduce((acc, [key, value]) => {
						...
						if (key.endsWith("_after") || key.endsWith("_before")) {
							acc[camelKey] = new Date((value as number) * 1000);
							...
							text: JSON.stringify(
								{
									...
									filters: Object.keys(params).filter((k) => k !== "project_id" && (params as any)[k] !== undefined),
									message: error.message || "Unknown error",
									code: error.code || "UNKNOWN",
								},
								null,
	 ...
	 */
	server.registerTool(
		"testrail_update_test_cases_batch",
		{
