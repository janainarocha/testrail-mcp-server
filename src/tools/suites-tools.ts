...
export function registerSuiteTools(server: McpServer, clients: { suites: TestRailSuitesAPI }) {
	/**
	...
	server.registerTool(
		...
		async ({ suite_id, name, description }) => {
			try {
				const suiteData: any = {};
				if (name) suiteData.name = name;
				if (description !== undefined) suiteData.description = description;
